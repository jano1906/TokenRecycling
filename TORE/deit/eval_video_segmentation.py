...
def eval_video_tracking_davis(args, model, frame_list, video_dir, first_seg, seg_ori, color_palette, kappa, device='cuda'):
    ...
    for cnt in tqdm(range(1, len(frame_list))):
        ...
        # upsampling & argmax
        frame_tar_avg = F.interpolate(frame_tar_avg, scale_factor=args.patch_size, mode='bilinear', align_corners=False,
...
def label_propagation(args, model, frame_tar, list_frame_feats, list_segs, kappa, mask_neighborhood=None, device='cuda'):
    ...
    if args.size_mask_neighborhood > 0:
        if mask_neighborhood is None:
            mask_neighborhood = restrict_neighborhood(h, w, device=device)
